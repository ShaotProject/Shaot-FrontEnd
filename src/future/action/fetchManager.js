...
export const getCompanyByIdFetch = (id) => (dispatch) => {
  ...
  fetch(`${baseURL}/company/${id}`, { method: "GET", mode: "no-cors" })
    .then((res) => res.json())
    .then((data) => {
      if (data.errorCode) {
...
export const getCompanySchedule = (id) => (dispatch) => {
  dispatch(loading(false));
  fetch(`${baseURL}/company/${id}/schedule`, { method: "GET", mode: "no-cors" })
    .then((res) => res.json())
...
export const fetchWeekNames = (id) => (dispatch) => {
  dispatch(loading(false));
  fetch(`${baseURL}/company/${id}/week/names`, { method: "GET", mode: "no-cors" })
    .then((res) => res.json())
